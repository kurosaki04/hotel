import { useState } from 'react';
import './style/sliderImag.css'

const SliderImag = ({hotel}) => {

    const [imgSelecter, setImgSelecter] = useState(0)

    console.log(hotel);

    const objStyle = {
        transform: `translate(calc(-${imgSelecter}/${hotel?.images.length} * 100%))`, 
        width: `calc(${hotel?.images.length} * 100%)`
    }

    const handlePrev = () => {
        if(imgSelecter !== 0) {
            setImgSelecter(imgSelecter - 1) 
         }
    }

    const handleNext = () => {
        if(imgSelecter + 1 <= hotel?.images.length - 1) {
           setImgSelecter(cv => cv + 1) 
        }
        
    }

  return (
    <div className="slider">
      <button onClick={handlePrev} className="slider__btn">&lt;</button>
      <div className="slider__exterior">
        <div style={objStyle} className="slider__interior">
          {
            hotel?.images.map(imgInfo => (
                <div className="slider__img-container" key={imgInfo.id}>
                    <img className="slider__img" src={imgInfo.url} alt="" />
                </div>
            ))
          }
        </div>
      </div>
      <button onClick={handleNext} className="slider__btn">&gt;</button>
    </div>
  )
}

export default SliderImag